...
This document contains supplementary material for research letter: *Canada: COVID-19 vaccine attitudes and motivations in adults with chronic health conditions* (Kosir et al. on behalf of the iCARE Study team).

The items presented are:

* Chi-square test to compare the vaccination rate between adults with and without chronic health conditions

* Figure 1 extended. The figure in the manuscript presents the 2 panels in which partially and fully vaccinated individuals are grouped together. To offer readers a further insight we broke it down into 3 panels, thus describing fully, partially, and non-vaccinated individuals and motivations.

***

### Chi-Square test
```{r}
# Chi square for the contingency table of vaccination between healthy and 
...
vax_rate_chi <- as.matrix(read.table(textConnection(vax_rate_chi),
                               header = T,
                               row.names = 1))
chisq.test(vax_rate_chi)
...
The figure in the manuscript is simplified and groups fully and partially vaccinated individuals because of their similar responses. However, here we present further breakdown by fully, partially, and non-vaccinated individuals.

**Proportion of individuals endorsing various vaccine motivators.** 
...
vaxed_fully <- leger_chronic %>%
  ...
  labs(title = "Motivations among fully vaccinated individuals (N = 1,343)", 
